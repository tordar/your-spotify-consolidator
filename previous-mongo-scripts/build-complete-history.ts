...
class MemoryEfficientHistoryBuilder {
  ...
  private async loadRawData(): Promise<void> {
    ...
    await this.loadDataFile<RawListeningEvent>('raw-mongo-data/raw_listening_history.json', (event) => {
      ...
      // ALWAYS collect dates for range calculation (regardless of track existence)
  ...
  private buildCompleteHistoryData(): CompleteListeningHistory {
    ...
    for (const [trackId, playCount] of this.trackPlayCounts) {
      ...
      // Sort listening events chronologically for this song
      const listeningEvents = (this.trackListeningEvents.get(trackId) || [])
    ...
    // Sort by earliest listening event (chronological order)
    completeSongs.sort((a, b) => {
      const aEarliest = new Date(a.listeningEvents[0]?.playedAt || '1970-01-01');
      const bEarliest = new Date(b.listeningEvents[0]?.playedAt || '1970-01-01');
  ...
  private saveCompleteHistory(history: CompleteListeningHistory): void {
    ...
    if (!fs.existsSync('complete-listening-history')) {
      fs.mkdirSync('complete-listening-history');
    }
